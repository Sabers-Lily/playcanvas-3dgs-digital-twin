...
export class GsplatMp4ProjectorAdapter {
  ...
  buildGlslPsChunk() {
    return [
      ...
      '  vec2 uv = vec2(0.0);',
      '  float hit = uProjectionMode > 0.5 ? gsplatProjectorQuadUv(worldPos, uv) : gsplatProjectorFrustumUv(worldPos, uv);',
      '  if (hit < 0.5) return;',
  ...
  updateUniforms() {
    ...
    material.setParameter('uProjectionMode', this.mode === PROJECTION_MODES.QUAD ? 1 : 0);
